...
class FlightController extends AdminController
{
    ...
    public function index(Request $request)
    {
        ...
        $data = [
            'rows'               => $query->with(['getAuthor', 'category_flight'])->paginate(20),
            'flight_categories'    => $this->flightCategoryClass::where('status', 'publish')->get()->toTree(),
    ...
    public function edit(Request $request, $id)
    {
        ...
        if (empty($row)) {
            return redirect('admin/module/flight');
        }
    ...
    public function bulkEdit(Request $request)
    {
        ...
        switch ($action) {
            case "delete":
                foreach ($ids as $id) {
                    $query = $this->flightClass::where("id", $id);
                    if (!$this->hasPermission('flight_manage_others')) {
                        $query->where("create_user", Auth::id());
            ...
            case "clone":
                $this->checkPermission('flight_create');
                foreach ($ids as $id) {
                    (new $this->flightClass())->saveCloneByID($id);
                }
