    ...
    <form action="{{route('flight.vendor.store',['id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}" method="post">
        ...
        <div class="form-add-service">
            <div class="nav nav-tabs nav-fill" id="nav-tab" role="tablist">
                <a data-toggle="tab" href="#nav-flight-content" aria-selected="true" class="active">{{__("1. Content")}}</a>
                <a data-toggle="tab" href="#nav-flight-location" aria-selected="false">{{__("2. Locations")}}</a>
                @if(is_default_lang())
                <a data-toggle="tab" href="#nav-flight-pricing" aria-selected="false">{{__("3. Pricing")}}</a>
            ...
            <div class="tab-content" id="nav-tabContent">
                <div class="tab-pane fade show active" id="nav-flight-content">
                    @include('Flight::admin/flight/flight-content')
                    @if(is_default_lang())
                ...
                <div class="tab-pane fade" id="nav-flight-location">
                    @include('Flight::admin/flight/flight-location',["is_smart_search"=>"1"])
                </div>
                @if(is_default_lang())
                <div class="tab-pane fade" id="nav-flight-pricing">
                    <div class="panel">
                    ...
                    @include('Flight::admin/flight/pricing')
                </div>
                <div class="tab-pane fade" id="nav-availability">
                    @include('Flight::admin/flight/availability')
                </div>
                <div class="tab-pane fade" id="nav-attribute">
                    @include('Flight::admin/flight/attributes')
                </div>
                <div class="tab-pane fade" id="nav-ical">
                    @include('Flight::admin/flight/ical')
                </div>
                @endif
            </div>
        </div>
