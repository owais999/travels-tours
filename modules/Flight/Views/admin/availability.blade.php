...
<div class="container-fluid">
    <div class="d-flex justify-content-between mb20">
        <h1 class="title-bar">{{__("Flights Availability Calendar")}}</h1>
    </div>
    ...
    @else
    <div class="alert alert-warning">{{__("No flights found")}}</div>
    @endif
...
<script>
    ...
    $('#items_tab').on('show.bs.tab', function(e) {
        ...
        calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            events: {
                url: "{{route('flight.admin.availability.loadDates')}}",
                extraParams: {
                    id: lastId,
                }
            },
            ...
            eventRender: function(info) {
                ...
                $(info.el).find('.fc-content').attr("data-html", "true").attr("title", info.event.title).tooltip({
                    boundary: 'window'
                })
            }
        });
    ...
    formModal = new Vue({
        ...
        methods: {
            ...
            saveForm: function() {
                ...
                $.ajax({
                    url: "{{route('flight.admin.availability.store')}}",
                    data: this.form,
                    dataType: 'json',
                    method: 'post',
