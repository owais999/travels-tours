...
class Flight extends Bookable
{
     ...
     */
    public function category_flight()
    {
        return $this->hasOne("Modules\Flight\Models\FlightCategory", "id", 'category_id')->with(['translations']);
    }
    ...
    static public function getSeoMetaForPageList()
    {
        ...
        } else if (!empty($title = setting_item_with_lang("flight_page_search_title"))) {
            $meta['seo_title'] = $title;
        }
        $meta['seo_image'] = null;
        if (!empty($title = setting_item("flight_page_list_seo_image"))) {
            $meta['seo_image'] = $title;
        } else if (!empty($title = setting_item("flight_page_search_banner"))) {
            $meta['seo_image'] = $title;
        }
    ...
    {
        return $this->hasOne($this->flightMetaClass, "flight_id");
    }
     ...
     */
    public function flight_term()
    {
        return $this->hasMany($this->flightTermClass, "flight_id");
    }
    ...
    public function getDetailUrl($include_param = true)
    {
        ...
        $urlDetail = app_get_locale(false, false, '/') . config('flight.flight_route_prefix') . "/" . $this->slug;
        if (!empty($param)) {
    ...
    {
        return url('admin/module/flightI/edit/' . $this->id);
    }
    ...
    {
        $meta = $this->flightMetaClass::where('flight_id', $this->id)->first();
        if (!$meta) {
            $meta = new $this->flightMetaClass();
            $meta->flight_id = $this->id;
        }
        $arg = $request->input();
    ...
    public function addToCart(Request $request)
    {
        ...
        $list_fees = setting_item('flight_booking_buyer_fees');
        $total_fee = 0;
        if (!empty($list_fees)) {
    ...
    {
        $datesRaw = $this->flightDateClass::getDatesInRanges($start_date, $this->id);
        $dates = [
            'base_price' => null,
            'person_types' => null,
        ];
    ...
    {
        $flightDate = $this->flightDateClass::where('target_id', $this->id)->where('start_date', $start_date)->where('active', 1)->first();
        ...
        $maxGuests = !empty($flightDate->max_guests) ? $flightDate->max_guests : $this->max_people;
        ...
        if ($meta) {
            ...
            if ($meta->enable_open_hours) {
                ...
                if (!isset($open_hours[$nDate]) or empty($open_hours[$nDate]['enable'])) {
                    return $this->sendError(__("This flight is not open on your selected day"));
                }
            }
        }
    ...
    public function getReviewDataAttribute()
    {
        ...
        $dataTotalReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id', $this->id)->where('object_model', "flight")->where("status", "approved")->first();
    ...
    public function getNumberServiceInLocation($location)
    {
        ...
        if (!empty($location)) {
            $number = parent::join('bravo_locations', function ($join) use ($location) {
                $join->on('bravo_locations.id', '=', 'bravo_flights.location_id')->where('bravo_locations._lft', '>=', $location->_lft)->where('bravo_locations._rgt', '<=', $location->_rgt);
            })->where("bravo_flights.status", "publish")->with(['translations'])->count("bravo_flights.id");
        }
    ...
    public function getBookingsInRange($from, $to)
    {
        ...
        $query->where('object_model', 'flight');
    ...
    public function saveCloneByID($clone_id)
    {
        ...
        $selected_terms = $old->flight_term->pluck('term_id');
        ...
        //Language
        $langs = $this->flightTranslationClass::where("origin_id", $old->id)->get();
        if (!empty($langs)) {
        ...
        //Meta
        $metaFlight = $this->flightMetaClass::where('flight_id', $old->id)->first();
        if (!empty($metaFlight)) {
            $metaFlightNew = $metaFlight->replicate();
            $metaFlightNew->flight_id = $new->id;
            $metaFlightNew->save();
        }
    }
    ...
    {
        $model_Flight = parent::query()->select("bravo_flights.*");
        ...
        if (!empty($price_range = $request->query('price_range'))) {
            ...
            $raw_sql_min_max = "( (IFNULL(bravo_flights.sale_price,0) > 0 and bravo_flights.sale_price >= ? ) OR (IFNULL(bravo_flights.sale_price,0) <= 0 and bravo_flights.price >= ?) ) 
								AND ( (IFNULL(bravo_flights.sale_price,0) > 0 and bravo_flights.sale_price <= ? ) OR (IFNULL(bravo_flights.sale_price,0) <= 0 and bravo_flights.price <= ?) )";
        ...
        if (is_array($terms) && !empty($terms)) {
            $model_Flight->join('bravo_flight_term as tt', 'tt.flight_id', "bravo_flights.id")->whereIn('tt.term_id', $terms);
        }
        $review_scores = $request->query('review_score');
        if (is_array($review_scores) && !empty($review_scores)) {
            $where_review_score = [];
            foreach ($review_scores as $number) {
                $where_review_score[] = " ( bravo_flights.review_score >= {$number} AND bravo_flights.review_score <= {$number}.9 ) ";
            }
            $sql_where_review_score = " ( " . implode("OR", $where_review_score) . " )  ";
            $model_Flight->WhereRaw($sql_where_review_score);
        }

        if (!empty($service_name = $request->query("service_name"))) {
            if (setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale()) {
                $model_Flight->leftJoin('bravo_flight_translations', function ($join) {
                    $join->on('bravo_flights.id', '=', 'bravo_flight_translations.origin_id');
                });
                $model_Flight->where('bravo_flight_translations.title', 'LIKE', '%' . $service_name . '%');
            } else {
                $model_Flight->where('bravo_flights.title', 'LIKE', '%' . $service_name . '%');
            }
        }

        $model_Flight->orderBy("is_featured", "desc");
        $model_Flight->orderBy("id", "desc");
        $model_Flight->groupBy("bravo_flights.id");

        $limit = min(20, $request->query('limit', 9));
        return $model_Flight->with(['location', 'hasWishList', 'translations'])->paginate($limit);
    }
}
